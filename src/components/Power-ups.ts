...
export class PowerUp{
    ...
    constructor(ctx : CanvasRenderingContext2D, posX : number, posY : number, powerUpOption : number){
        this.ctx = ctx;
    ...
    draw(){
        ...
        this.ctx.beginPath();
        this.ctx.drawImage(POWER_UPS[this.powerUpOption], this.posX, this.posY, this.width, this.height);
        this.ctx.closePath();        

    }
    update(){
        if(this.posY<CANVAS_DIMENSIONS.CANVAS_HEIGHT - GROUND_HEIGHT-this.height){
